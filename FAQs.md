...
A) No. The `resume.json` is embedded in the WASM binary via Go's `//go:embed` directive. Search engine crawlers don't execute WebAssembly, so the content inside `mk.wasm` would not be visible to them. Only the static HTML in index.html would be indexed — and that contains no resume content, just the terminal UI shell.